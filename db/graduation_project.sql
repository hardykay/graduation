...
CREATE TABLE `tp_alteration` (
  ...
  `department` text COMMENT '系主任的意见',
  `department_audit` tinyint(4) DEFAULT '0' COMMENT '系主任审核结果，0未审核，1通过审核，2退回修改',
  `college` text COMMENT '教学院长审核结果',
  `college_audit` tinyint(4) DEFAULT '0' COMMENT '教学院长审核结果，0未审核，1通过审核，2退回修改',
  ...
  `dep_time` datetime DEFAULT NULL ON UPDATE CURRENT_TIMESTAMP COMMENT '系主任审核时间',
  `college_time` datetime DEFAULT NULL ON UPDATE CURRENT_TIMESTAMP COMMENT '教学院长审核时间',
  ...
  KEY `department_audit` (`department_audit`),
...
-- ----------------------------
-- Table structure for tp_member_two
-- ----------------------------
DROP TABLE IF EXISTS `tp_member_two`;
CREATE TABLE `tp_member_two` (
  `id` int(11) DEFAULT NULL COMMENT '答辩编号',
  `tea_number` varchar(20) DEFAULT NULL COMMENT '教师工号',
  KEY `id` (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='二辩-答辩组成员';

-- ----------------------------
-- Records of tp_member_two
-- ----------------------------
...
CREATE TABLE `tp_outside` (
  ...
  `shenhe2` tinyint(4) DEFAULT '0' COMMENT '专业负责人是否评阅0.1',
  `shenhe3` tinyint(4) DEFAULT '0' COMMENT '院长是否评阅0.1',
  `content` text COMMENT '申请理由',
  `content2` text COMMENT '校外指导老师意见',
  `content3` text COMMENT '校内指导老师意见',
  `content4` text COMMENT '专业负责人审核的意见',
  `content5` text COMMENT '院长意见',
  PRIMARY KEY (`id`),
  KEY `id` (`id`),
  KEY `stu_number` (`stu_number`),
  KEY `dep_college` (`dep_college`),
  ...
  KEY `shenhe2` (`shenhe2`),
  KEY `shenhe3` (`shenhe3`)
) ENGINE=MyISAM DEFAULT CHARSET=utf8 COMMENT='校外毕业设计申请表';
...
-- ----------------------------
INSERT INTO `tp_specialty` VALUES ('1', '计科', '1');
...
CREATE TABLE `tp_squad_stu` (
  ...
  `squad` tinyint(4) DEFAULT '0' COMMENT '是否有答辩小组0没答辩，1，已经通过，2进入二辩，3进入优秀',
  KEY `id` (`id`),
  KEY `dep_id` (`dep_id`)
...
-- ----------------------------
-- Table structure for tp_squad_stu_two
-- ----------------------------
DROP TABLE IF EXISTS `tp_squad_stu_two`;
CREATE TABLE `tp_squad_stu_two` (
  `id` int(11) NOT NULL DEFAULT '0' COMMENT '答辩小组编号',
  `stu_number` char(20) NOT NULL COMMENT '学号',
  `dep_id` int(11) NOT NULL DEFAULT '0' COMMENT '专业编号',
  `squad` tinyint(4) DEFAULT '0' COMMENT '是否有答辩小组0没答辩，1答辩',
  KEY `id` (`id`),
  KEY `dep_id` (`dep_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='二辩-答辩学生';

-- ----------------------------
-- Records of tp_squad_stu_two
-- ----------------------------

-- ----------------------------
-- Table structure for tp_squad_two
-- ----------------------------
DROP TABLE IF EXISTS `tp_squad_two`;
CREATE TABLE `tp_squad_two` (
  `id` int(11) unsigned NOT NULL AUTO_INCREMENT COMMENT '编号',
  `dep_id` int(11) NOT NULL COMMENT '学院编号',
  `zz_name` char(20) NOT NULL COMMENT '答辩组长',
  `zz_number` char(20) NOT NULL COMMENT '组长工号',
  `tea_number` char(20) DEFAULT NULL COMMENT '答辩秘书教师工号',
  `tea_name` char(20) DEFAULT NULL COMMENT '答辩秘书姓名',
  ...
  KEY `id` (`id`),
  KEY `dep_id` (`dep_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='二辩-答辩小组';

-- ----------------------------
-- Records of tp_squad_two
-- ----------------------------
...
CREATE TABLE `tp_student` (
  ...
  `stu_type` tinyint(4) NOT NULL DEFAULT '0' COMMENT '选中的课题类型，0未选，1盲选，2指定选、3团队选，4外出毕设',
...
-- ----------------------------
INSERT INTO `tp_teacher` VALUES ('admin', '管理员', 'a72439cf182ba314255f6a5bd39f4f34', null, '', '', null, '', '', '', '', '1755808168@qq.com', '', '1', '0', '0', '0', '0', '0');
...
CREATE TABLE `tp_topic` (
  ...
  `rele` tinyint(4) DEFAULT '0' COMMENT '是否发布0没发布，1发布（可以选题），2发布，学生可以查看结果，但不可以选题',
  `first` tinyint(4) DEFAULT '0' COMMENT '0表示还未审核，1表示通过审核即有一位或多位专业负责人审核，2表示退回修改',
  `opt` tinyint(4) DEFAULT '1' COMMENT '可否选择该可以0表示不可以，1表示可以',
  `top_type` tinyint(4) DEFAULT NULL COMMENT '课题类型，1盲选，2指定选、3团队课题，4外出毕设',
  `audit` tinyint(4) DEFAULT '0' COMMENT '审核状态，0未审核，1通过审核，2退回修改',
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `tp_view_topic` AS select `tp_topic_f`.`top_id` AS `top_id`,`tp_topic_f`.`dep_id` AS `zy`,`tp_topic_f`.`audit` AS `zy_audit`,`tp_topic`.`name` AS `name`,`tp_topic`.`tea_number` AS `tea_number`,`tp_topic`.`tea_name` AS `tea_name`,`tp_topic`.`genre` AS `genre`,`tp_topic`.`seientific` AS `seientific`,`tp_topic`.`practice` AS `practice`,`tp_topic`.`content` AS `content`,`tp_topic`.`rele` AS `rele`,`tp_topic`.`first` AS `first`,`tp_topic`.`opt` AS `opt`,`tp_topic`.`top_type` AS `top_type`,`tp_topic`.`youxiao` AS `youxiao`,`tp_topic`.`audit` AS `audit`,`tp_topic`.`dep_id` AS `dep_id` from (`tp_topic_f` join `tp_topic`) where (`tp_topic`.`top_id` = `tp_topic_f`.`top_id`) ;
