...
class SquadController extends Controller {
    ...
    public function index() {
        ...
        //tp_specialty
        $spe = "select dep_id,dep_name from tp_specialty where dep_id in( $dep )";
...
//            //p($data);
    ...
    public function add($dep = 0) {
        ...
        if ($flag) {
            $Model->commit();
            $this->success('成功添加', U('/Director/Squad/Squad/dep/' . $dep));
        } else {
            $Model->rollback();
            $this->error('添加失败,请联系管理员');
        }
...
//        is_numeric($id) ? 0 : $this->error('请求超时.');
        ...
        $this->assign('data', $data);
        $this->assign('list', $result);
        $page = getPageHtml($page, $count, __ROOT__ . '/Director/Squad/Teacher/id/' . $id . '/dep/' . $dep_id);
    ...
    public function student($id, $dep, $page = 0) {
        ...
        $data['dep'] = $dep;
        $this->assign('data', $data);
        $page = getPageHtml($page, $count, __ROOT__ . "/Director/Squad/Student/id/{$id}/dep/{$dep}");
        $this->assign('page', $page);
        $this->display();
        /* //防止其他人进入这个页面，符合这个条件的只有系主任
          $d = explode(',', session('dep'));
          in_array($dep, $d) ? 1 : $this->error('访问页面不存在', C('error_go'));
          //squad=1 or 0 分别代表通过，和没有答辩，这些人不能给以再分配答辩小组。2,3分别代表二辩及优秀答辩，这些人需要再次分配
          $sql = M('Squad_stu')->field('stu_number')->where('squad!=0 and dep_id=' . $dep)->group('stu_number')->buildSql();
          //将不符合条件的学生排除在外
          //        $count = M('view_std')->where('dep_major=' . $dep . ' and stu_number not in ' . $sql . ' and tea_number not in ' . $stu)
          $count = M('view_std')->where('dep_major=' . $dep . ' and stu_number not in ' . $sql)
          ->count();
          $pagenum = ($page < 2 || (($page - 1) * 10) > $count) ? 0 : ($page - 1) * 10;
          //        $result = M('view_std')->where('dep_major=' . $dep . ' and stu_number not in ' . $sql . ' and tea_number not in ' . $stu)
          $result = M('view_std')->where('dep_major=' . $dep . ' and stu_number not in ' . $sql)
